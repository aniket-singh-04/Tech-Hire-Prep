...
} from "../models/interviewSession.model.ts";
...
const notifyPendingRequestsForUser = async (userId: string) => {
  ...
  for (const request of activeRequests) {
    const requesterProfile = await profileRepository.findByUserId(
    ...
    const eligible = isUserEligibleForRequest(
      ...
      {
        profileCompletion: requesterProfile.profileCompletion,
        targetRole: String(request.preferredRole),
      },
      {
        profileCompletion: candidateProfile.profileCompletion,
        targetRole: String(candidateProfile.targetRole),
        availability: candidateProfile.availability as
          | Array<{ day: string; startTime: string; endTime: string }>
          | undefined,
      },
      busyUserIds,
    ...
    const alreadyNotified = request.notifiedUsers?.some(
      (entry) =>
        entry.userId.toString() === userId && entry.status === "PENDING",
    );

    if (!alreadyNotified) {
      await MatchRepository.addNotifiedUser(
...
export const initSocketServer = (server: HttpServer) => {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("webrtc-offer", (data: { roomId: string; offer: any }) => {
      socket
        .to(data.roomId)
        .emit("webrtc-offer", { offer: data.offer, senderId: userId });
    });

    socket.on("webrtc-answer", (data: { roomId: string; answer: any }) => {
      socket
        .to(data.roomId)
        .emit("webrtc-answer", { answer: data.answer, senderId: userId });
    });
