...
export const createAccessToken = (payload: { userId: string; role: UserRole }) => {
  ...
  const body = encode({ sub: payload.userId, role: payload.role, type: "access", iat: now, exp: now + ENV.ACCESS_TOKEN_TTL_SECONDS } satisfies AccessTokenPayload);
...
export const verifyAccessToken = (token: string): AccessTokenPayload => {
  ...
  if (!header || !body || !signature) throw new AppError("Unauthorized", 401);
...
export const hashRefreshToken = (token: string) => createHmac("sha256", ENV.REFRESH_TOKEN_SECRET).update(token).digest("base64url");
export const refreshTokenExpiry = () => new Date(Date.now() + ENV.REFRESH_TOKEN_TTL_DAYS * 24 * 60 * 60 * 1000);
